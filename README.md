...
\*\*DAG name:\*\* `bronze\_openmeteo\_daily\_json`
...
(location\_name, start\_dt, end\_dt, source, upsert\_ts)

\- `upsert\_ts` allows safe re-runs, backfills, and corrections
...
\*\*Table:\*\* `bronze\_openmeteo\_raw`



Stores raw JSON payloads (`hourly\_json`, `daily\_json`) along with metadata:
